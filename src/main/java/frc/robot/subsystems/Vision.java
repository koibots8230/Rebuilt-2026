...
public class Vision extends SubsystemBase {
  ...
  private Pose2d translateToFieldPose(
    ...
    Rotation2d angle =
        new Rotation2d(
            Math.PI
                + VisionConstants.CAMERA_POSITIONS[camera].getRotation().getZ()
  ...
  private boolean withinDistance(double[] translation) {
    return Math.hypot(translation[0], translation[2]) < VisionConstants.MAX_TAG_DISTANCE.in(Meters);
  }

  private boolean withinHeight(double[] translation, int id, int camera) {
    return Math.abs(
            layout.getTagPose(id).get().getTranslation().getZ()
                - translation[1]
                - VisionConstants.CAMERA_POSITIONS[camera].getZ())
        < VisionConstants.MAX_HEIGHT_ERROR.in(Meters);
  }
        ...
        if (ids[b].value != 0
          ...
          if (withinField(pose)
              && ((RobotState.isDisabled())
                  ? true
                  : Math.hypot(
                          pose.getX() - getOdometry.get().getX(),
                          pose.getY() - getOdometry.get().getY())
                ...
                new VisionMeasurement(
                    ...
                    (RobotState.isDisabled()
                        ...
                        : Math.pow(
                            ...
                            * VisionConstants.CAM_STDEV_SCALARS[a]),
